...
const data = shelters as Resource[];
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-heading text-4xl font-bold text-warm-900">{tr.shelter.title}</h1>
        </div>
        <p className="text-lg text-warm-600 max-w-2xl leading-relaxed">{tr.shelter.subtitle}</p>
      </div>
      <div className="bg-amber-50 border-l-4 border-amber-500 rounded-r-xl px-4 py-3 mb-8">
        <p className="text-amber-900 font-semibold text-base">
          Shelter availability changes daily — always call ahead, or text <strong>211</strong> to check what's open right now.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filtered.map(r => <ResourceCard key={r.id} resource={r} accentColor="border-l-sky-500" />)}
      </div>
    </div>
  );
}
